...
#else
				if (!(trigger->spawnflags & RESTRICT_FLAG_BHOP) && (player->client->ps.stats[STAT_RESTRICTIONS] & JAPRO_RESTRICT_BHOP)) {
					player->client->ps.stats[STAT_RESTRICTIONS] &= ~JAPRO_RESTRICT_BHOP;
				}
#endif
			}
...
#if 0//egh fuck, this was supposed to slowly heal players in the hill from whichever team had more players in it
	//i think for this to work it needs to actually keep count of how many from each team are inside of the trigger, not sure how to do that rn
	if (g_KOTH.integer >= 2 && (level.kothTime < -1500 || level.kothTime > 1500) &&
		!(other->s.eFlags & EF_DEAD))
	{
		int hp = 100 / addTime;
		if (hp < 0) hp *= -1;

		if (other->health < other->playerState->stats[STAT_MAX_HEALTH])
			other->health += hp;
		else if (g_KOTH.integer >= 3 && other->client->damage_armor < 200)
			other->client->damage_armor += hp;
		else if (other->client->damage_armor < 100)
			other->client->damage_armor += hp;

		if (other->health > other->client->ps.stats[STAT_HEALTH]) other->client->ps.stats[STAT_HEALTH] = other->health;
		if (other->client->damage_armor > other->client->ps.stats[STAT_ARMOR]) other->client->ps.stats[STAT_ARMOR] = other->client->damage_armor;
	}
#endif
...
#if 0
	if (g_KOTH.integer >= 2) {
		vec3_t center = { self->r.currentOrigin[0], self->r.currentOrigin[1], self->r.currentOrigin[2] };
		float centerDistance;
		if (g_KOTH.integer == 2)
			center[2] = other->r.currentOrigin[2];

		centerDistance = Distance(center, other->r.currentOrigin);

		if (centerDistance) {
			//Com_Printf("centerDistance %f \n", centerDistance);
		}
		/*Com_Printf("self->r.currentOrigin %f %f %f other->r.currentOrigin %f %f %f \n",
			self->r.currentOrigin[0], self->r.currentOrigin[1], self->r.currentOrigin[2],
			other->r.currentOrigin[0], other->r.currentOrigin[1], other->r.currentOrigin[2]);*/
	}
#endif
