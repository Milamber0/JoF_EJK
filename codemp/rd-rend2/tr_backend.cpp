		...
		switch ( depthRange )
		{
			...
			case 1:
				if ( backEnd.viewParms.stereoFrame != STEREO_CENTER )
				{
					...
					//R_SetupProjection(&temp, r_znear->value, 0, qfalse); //bucky
					R_SetupProjection(&temp, r_znear->value*tr.widthRatioCoef, 0, qfalse);
...
static void RB_RenderSunShadows()
{
	...
	//const float zmin = r_znear->value; //bucky
	const float zmin = r_znear->value*tr.widthRatioCoef;
...
#elif 0 //attempt at merging somaz changes with original code....
static void RB_RenderSSAO()
{
	const float zmax = backEnd.viewParms.zFar;
	//const float zmin = r_znear->value; //bucky
