...
void R_CreateBuiltinImages( void ) {
	...
	if (r_ssao->integer)
	{
		if (r_ssao->integer == 1) {
			tr.screenSsaoImage = R_CreateImage(
				"*screenSsao", NULL, width, height, IMGTYPE_COLORALPHA,
				IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_RGBA8);
		}
		else {
			tr.screenSsaoImage = R_CreateImage(
...
void R_SetColorMappings( void ) {
	...
	else if (tr.world && VALIDSTRING(tr.worldDir) && !r_overBrightBits->modified)
	{
		if (!Q_stricmpn(tr.worldDir, "maps/mp/ffa3", 12) || !Q_stricmpn(tr.worldDir, "maps/ffa3", 9) || !Q_stricmpn(tr.worldDir, "maps/kor2", 9) || !Q_stricmpn(tr.worldDir, "maps/t3_stamp", 13)
