...
#if 0
	if (1 || ent->e.renderfx & RF_NOLOD) {
		Com_Printf("startGridPos %i gridStep %i %i %i ent->ambientLight %.0f %.0f %.0f ent->directedLight %.0f %.0f %.0f\n",
			(int)startGridPos, gridStep[0], gridStep[1], gridStep[2], ent->ambientLight[0], ent->ambientLight[1], ent->ambientLight[2],
