...
#if 0
const void RB_DrawScaledSceneImage(void) {
	qglDisable(GL_CLIP_PLANE0);
	...
	glScissor(0, 0, r_scaledWidth->integer, r_scaledHeight->integer);
	// Render the normal scene texture.
	qglBindTexture(GL_TEXTURE_RECTANGLE_ARB, tr.sceneImage);
	qglBegin(GL_QUADS);
		qglColor4f( 1.0f, 1.0f, 1.0f, 1.0f );
		qglTexCoord2f( 0, r_scaledHeight->integer );
		qglVertex2f( 0, 0 );

		qglTexCoord2f( 0, 0 );
		qglVertex2f( 0, glConfig.vidHeight );

		qglTexCoord2f( r_scaledWidth->integer, 0 );
		qglVertex2f( glConfig.vidWidth, glConfig.vidHeight );

		qglTexCoord2f( r_scaledWidth->integer, r_scaledHeight->integer );
